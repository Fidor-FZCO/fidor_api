...
describe FidorApi::Beneficiary do
  ...
  def expect_correct_beneficiary(beneficiary)
    ...
    expect(beneficiary.contact_name).to           eq "Emad Hussain"
    ...
    expect(beneficiary.routing_type).to           eq "UAE_DOMESTIC"
    expect(beneficiary.routing_info).to           be_a Hash
    expect(beneficiary.verified).to               be true

    expect(beneficiary.routing_info["account_number"]).to eq "AE123456789012345678"
    expect(beneficiary.routing_info["swift_code"]).to     eq "ABDIAEAD"
  end
