module FidorApi
  module CustomerConfirmations
    ...
    def create_confirmation_token(type, resource)
      Connectivity::Connection.post(END_POINT, body: build_payload(type, resource), anonymous: true, access_token: nil)
    end

    def confirm_resource(type, resource, code)
      Connectivity::Connection.put(END_POINT, body: build_payload(type, resource, code), anonymous: true, access_token: nil)
    end
